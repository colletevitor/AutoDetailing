...
import { Car, CalendarDays, Users, Wrench, TrendingUp, Menu, X } from "lucide-react";
      ...
      <header className="sticky top-0 z-40 bg-[#0A0A0B]/90 backdrop-blur border-b border-[#27272A]">
        <div className="max-w-7xl mx-auto px-4 sm:px-6">
          <div className="flex items-center justify-between h-14 sm:h-16">
            ...
            {/* Mobile hamburger */}
            <button
              className="sm:hidden w-9 h-9 flex items-center justify-center rounded-lg hover:bg-[#27272A] transition-colors"
        ...
        {TABS.map(({ id, label, icon: Icon }) => (
          <button
            ...
            className={`flex flex-col items-center gap-1 py-2 text-[10px] font-medium transition-colors ${
              activeTab === id ? "text-[#F97316]" : "text-[#52525B]"
            }`}
          >
            <Icon size={20} />
            {label}
