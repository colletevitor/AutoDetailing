...
import { Plus, ChevronLeft, ChevronRight, Car, X, Check, Clock, Loader2 } from "lucide-react";
import { Appointment, Service, Employee, ServiceStatus } from "../types";
import { getAppointments, createAppointment, updateAppointment, deleteAppointment, getServices, getEmployees } from "../lib/db";
...
const EMPTY_FORM = { clientName: "", carPlate: "", carModel: "", serviceId: "", employeeId: "", date: "", time: "09:00", notes: "" };
...
export default function AgendaTab() {
  ...
  function openEdit(a: Appointment) { setEditing(a); setForm({ clientName: a.clientName, carPlate: a.carPlate, carModel: a.carModel, serviceId: a.serviceId, employeeId: a.employeeId, date: a.date, time: a.time, notes: a.notes ?? "" }); setModal("edit"); }
  ...
  async function handleSave() {
    ...
    setSaving(true);
    if (modal === "add") {
      await createAppointment({ ...form, status: "aguardando" });
  ...
  async function handleStatusChange(status: ServiceStatus) {
    if (!editing) return;
    const completedAt = status === "concluido" ? new Date().toISOString() : editing.completedAt;
    await updateAppointment({ ...editing, status, completedAt });
    await load();
    close();
  }
  ...
  return (
    <div>
      ...
      <div className="flex items-center gap-3 mb-6">
        <button onClick={() => shiftDate(-1)} className="w-9 h-9 flex items-center justify-center rounded-lg bg-[#111113] border border-[#27272A] hover:border-[#3F3F46] transition-colors"><ChevronLeft size={18} /></button>
        <div className="flex-1 text-center">
          <p className="font-bold capitalize">{isToday ? "Hoje" : weekday}</p>
          <p className="text-[#A1A1AA] text-sm">{fmtDate(selectedDate)}</p>
        </div>
        <button onClick={() => shiftDate(1)} className="w-9 h-9 flex items-center justify-center rounded-lg bg-[#111113] border border-[#27272A] hover:border-[#3F3F46] transition-colors"><ChevronRight size={18} /></button>
      </div>
              ...
              <div key={a.id} className="bg-[#111113] border border-[#27272A] rounded-xl overflow-hidden hover:border-[#3F3F46] transition-colors">
                ...
                <div className="p-4">
                  ...
                  <div className="flex flex-wrap gap-x-4 gap-y-1.5 text-xs text-[#A1A1AA]">
                    ...
                    {emp && <span className="flex items-center gap-1.5"><span className="w-2.5 h-2.5 rounded-full inline-block" style={{ backgroundColor: emp.color }} />{emp.name}</span>}
          ...
          <div className="bg-[#111113] border border-[#27272A] rounded-2xl w-full max-w-sm">
            ...
            <div className="p-4 space-y-2">
              {STATUSES.map(([key, cfg]) => (
                <button key={key} onClick={() => handleStatusChange(key)} className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl transition-colors ${editing.status === key ? `${cfg.bg} ${cfg.text}` : "hover:bg-[#27272A] text-[#A1A1AA]"}`}>
