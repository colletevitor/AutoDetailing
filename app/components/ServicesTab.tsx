...
import { Plus, Pencil, Trash2, Wrench, X, Check, Loader2 } from "lucide-react";
            ...
            <div key={s.id} className="bg-[#111113] border border-[#27272A] rounded-xl p-4 hover:border-[#3F3F46] transition-colors">
              <div className="flex items-start justify-between gap-2 mb-3">
                ...
                <div className="flex gap-1 shrink-0">
                  <button onClick={() => openEdit(s)} className="w-7 h-7 flex items-center justify-center rounded-lg hover:bg-[#27272A] text-[#A1A1AA] hover:text-[#FAFAFA] transition-colors"><Pencil size={13} /></button>
                  <button onClick={() => handleDelete(s.id)} className="w-7 h-7 flex items-center justify-center rounded-lg hover:bg-red-500/15 text-[#A1A1AA] hover:text-red-400 transition-colors"><Trash2 size={13} /></button>
                </div>
              </div>
